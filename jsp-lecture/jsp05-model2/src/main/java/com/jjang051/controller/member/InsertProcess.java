...
public class InsertProcess extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * request.getParameter("userName"); Part profile = request.getPart("profile");
		 * 
		 * int postCode = 0; if(request.getParameter("postCode")!=null &&
		 * !request.getParameter("postCode").isEmpty()) { postCode =
		 * Integer.parseInt(request.getParameter("postCode")); } String address =
		 * request.getParameter("address"); String detailAddress =
		 * request.getParameter("detailAddress"); String email =
		...
		if(!originalFileName.isEmpty()) {
			...
			profile.write(originalFileName);
		}
